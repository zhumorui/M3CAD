...
def _fill_trainval_infos(nusc,
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        info['sweeps'] = sweeps
        # obtain annotations
        # Here we also get annotations from test dataset for evaluation in carla_ue5 dataset
        annotations = [
...
def obtain_sensor2top(nusc,
    ...
    # Create initial sweep dictionary, storing sensor to Ego pose information
    sweep = {
        'data_path': data_path,
        'type': sensor_type,
    ...
    # Read sensor pose relative to ego vehicle
    ...
    # Directly calculate rotation and translation matrix from sensor to LiDAR
    R = l2e_r_s_mat.T @ np.linalg.inv(l2e_r_mat).T
    T = (l2e_t_s @ np.linalg.inv(l2e_r_mat).T) - l2e_t @ np.linalg.inv(l2e_r_mat).T

    # Store rotation and translation from sensor to LiDAR
