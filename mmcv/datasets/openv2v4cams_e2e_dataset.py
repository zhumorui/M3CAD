...
class OpenV2V4CAMSE2EDataset(NuScenesDataset):
    ...
    def prepare_test_sender_data(self, i):
        ...
        try:
            ...
            if not match:
                self._update_test_transformation(ego_example, ego_dict['can_bus'])
                return ego_example
            ...
            # If no suitable sender found, return ego_example
            if not rotation_diffs:
                self._update_test_transformation(ego_example, target_can_bus)
                return ego_example
            ...
            self._update_test_transformation(sender_example, target_can_bus)
    ...
    def _update_test_transformation(self, example, target_can_bus):
        """
        Update transformation parameters safely
        ...
        try:
            ...
            can_bus_data = copy.deepcopy(example['img_metas'][0].data['can_bus'])

        if len(target_can_bus) < 3 or len(can_bus_data) < 6:
            raise ValueError("Input arrays must have sufficient length")
            
        sender_pos = example['img_metas'][0].data['can_bus'][:2]
        sender_angle = example['img_metas'][0].data['can_bus'][-1]
        ...
        example['img_metas'][0].data['can_bus'] = can_bus_data
    ...
    def prepare_test_data(self, index):
        ...
        if self.enable_fusion:
            ...
            if sender_data_queue:
                ...
                data_dict['sender_gt_bboxes_3d'] = sender_data_queue_out.get('gt_bboxes_3d', None)
        return data_dict
