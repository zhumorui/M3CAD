...
from mmcv.fusion_utils import visualize_bev, transform_coordinates
...
class UniADTrack(MVXTwoStageDetector):
    ...
    def __init__(
        ...
        if self.enable_fusion:
            if fusion_cfg is not None:
                ...
                if fusion_model_name == "F_Cooper":
                    self.feature_fusion = get_fusion_model(fusion_model_name)
                elif fusion_model_name == "Cobev":
    ...
    def fusion_bev_features(self, bev_embed, sender_bev_embed, sender_mask):
        ...
        if self.fusion_model_name == "F_Cooper":
            record_len = torch.tensor([2], device=input_bev.device)
            fusion_bev_embed = self.feature_fusion(input_bev, record_len)
        elif self.fusion_model_name == "Cobev":
    ...
    def _forward_single_frame_inference(
        ...
        if self.debug:
            if self.enable_fusion:
                visualize_bev(bev_embed, backup_sender_bev, sender_bev_embed, fusion_bev_embed, gt_bboxes_3d, sender_gt_bboxes_3d, sender_img_metas, vis_idx=0, debug=self.debug)
